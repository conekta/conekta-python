...
class OrderCheckoutRequest(BaseModel):
    ...
    """ # noqa: E501
    allowed_payment_methods: List[StrictStr] = Field(description="Are the payment methods available for this link. For subscriptions, only 'card' is allowed due to the recurring nature of the payments.", json_schema_extra={"examples": [["cash", "card", "bank_transfer", "bnpl", "pay_by_bank", "google", "apple"]]})
    ...
    __properties: ClassVar[List[str]] = ["allowed_payment_methods", "exclude_card_networks", "plan_ids", "expires_at", "failure_url", "force_save_card", "monthly_installments_enabled", "monthly_installments_options", "max_failed_retries", "name", "on_demand_enabled", "redirection_time", "success_url", "type"]

    @field_validator('allowed_payment_methods')
    def allowed_payment_methods_validate_enum(cls, value):
        """Validates the enum"""
        for i in value:
            if i not in set(['cash', 'card', 'bank_transfer', 'bnpl', 'pay_by_bank', 'google', 'apple']):
                raise ValueError("each list item must be one of ('cash', 'card', 'bank_transfer', 'bnpl', 'pay_by_bank', 'google', 'apple')")
        return value
